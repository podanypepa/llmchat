...
// Send implements the llm.Client interface for Deepseek.
func (c *Client) Send(ctx context.Context, req *llm.Request) (*llm.Response, error) {
	...
	for i, msg := range req.Messages {
		deepseekReq.Messages[i] = ChatMessage{
			Role:    msg.Role,
			Content: msg.Content,
		}
	}

	resp, err := c.send(ctx, deepseekReq)
	if err != nil {
		return nil, err
	}
...
// StreamSend implements the llm.Client interface for Deepseek.
func (c *Client) StreamSend(ctx context.Context, req *llm.Request) (<-chan llm.StreamChunk, error) {
	...
	deepseekStream, err := c.streamSend(ctx, deepseekReq)
	if err != nil {
		return nil, err
	}
